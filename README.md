...
一个面向 Airline Manager 4 Web 版的自托管机队仪表盘与自动运营工具。

它可以采集机队、市场、枢纽、需求、改装和检修信息，并通过持久化待办完成低价补货、营销续期、航线建设、改装和逐架起飞接管。项目针对 Realism 模式开发，目前版本为 **2.3.0**。
...
当天已成功完成全量采集时，服务重启会续接轻量循环；否则立即补做全量。在线异常采用有限退避，未知状态不会继续执行购机、建线、改装或起飞等写操作。
...
- 署名应至少包含“AM4 机队中心 by MQSI”、原项目仓库链接和本许可证链接；如有修改，应同时说明修改内容。
